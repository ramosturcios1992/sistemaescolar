@extends('layouts/app')
@section('titulo', 'Modificar Administrador')
@section('content')
    @if (Auth::user()->tipo == 1)
        @if (session('correcto'))
            <script>
        ...
        <div id="modal-edit">
            @foreach ($sql as $i)
                <h3 class="text-center" style="margin-bottom: 35px">MODIFICAR ADMINISTRADOR</h3>
                <form action="{{ route('directors.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
